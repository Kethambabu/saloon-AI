...
# Import warn_once to prevent repeated FAISS index-missing log spam (Bug 4 fix)
try:
...
# Roles permitted to query each domain directly (mirrors the CUSTOMER/STAFF/
# MANAGER+ split in ai/orchestrator.py's _ROLE_ALLOWED_INTENTS and
# mcp/permissions.py's ROLE_PERMISSIONS). POLICY_RAG and CUSTOMER_RAG are
# unrestricted here — CUSTOMER_RAG ownership is enforced separately in
# search_customer_rag(). This function isn't currently wired to a live agent
# tool (agents call infrastructure.rag.rag_unified.search_knowledge_base,
# which has its own equivalent gate), but is hardened the same way since it's
# a public entry point.
_DOMAIN_MIN_ROLES: Dict[RAGDomain, set] = {
    RAGDomain.STAFF_RAG:    {"STAFF", "MANAGER", "OWNER", "ADMIN"},
    RAGDomain.LEAD_RAG:     {"MANAGER", "OWNER", "ADMIN"},
    RAGDomain.BUSINESS_RAG: {"MANAGER", "OWNER", "ADMIN"},
}
        ...
        for idx_name in candidate_indices:
            ...
            if os.path.exists(faiss_file) or os.path.isdir(path):
                try:
                    ...
                    warn_once(
                        logger,
                        f"rag_load_fail_{idx_name}",
                        "[DomainRetriever] Failed to load %s: %s", path, exc,
                    )
        warn_once(
            logger,
            f"rag_no_index_{self.domain.value}_{self.tenant_id}",
...
class EnterpriseRAGManager:
    ...
    def search(
        ...
        required_roles = _DOMAIN_MIN_ROLES.get(domain)
        if required_roles:
            role = _current_role()
            if role not in required_roles:
                logger.warning(
                    "[RAGManager] Role '%s' denied access to domain=%s (requires one of %s)",
                    role, domain.value, sorted(required_roles),
                )
                return []
...
def search_policy_rag(query: str, tenant_id: str = "default", k: int = 3) -> str:
    ...
    try:
        ...
        if not results:
            ...
            try:
                from infrastructure.rag.receptionist_rag_tools import search_receptionist_knowledge
...
def search_customer_rag(
    ...
    try:
        # Force-override any explicitly supplied customer_id to the caller's
        # own identity for CUSTOMER role, mirroring mcp/query_guard.py's
        # handling of customer_id for CUSTOMER-role DB queries — otherwise a
        # CUSTOMER could pass another customer's UUID/name directly.
        if _current_role() == "CUSTOMER":
            ...
            if customer_id and str(customer_id) != str(own_id):
                logger.warning(
        ...
        if not results:
            try:
                from infrastructure.rag.retriever import search_customer_memory
    ...
    try:
        if staff_id:
            try:
                ...
                denial = check_staff_access(staff_id)
            except Exception:
                denial = None
            if denial:
                return denial
        ...
        if not results:
            try:
                from infrastructure.rag.retriever import search_staff_memory
...
def search_lead_rag(query: str, tenant_id: str = "default", k: int = 3) -> str:
    ...
    try:
        ...
        if not results:
            try:
                from infrastructure.rag.retriever import search_lead_memory
...
def search_business_rag(query: str, tenant_id: str = "default", k: int = 3) -> str:
    ...
    try:
        ...
        if not results:
            try:
                from infrastructure.rag.retriever import search_bi_memory
...
def search_knowledge_base_v2(
    ...
    if rag_domain is None and domain_lower != "all_context":
        ...
        try:
            from infrastructure.rag.rag_unified import search_knowledge_base as _legacy_skb
