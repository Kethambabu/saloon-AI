...
def seed_database():
    ...
    try:
        ...
        if marcus_st:
            m_leave = StaffLeave(
                id=uuid.uuid4(),
                staff_id=marcus_st.id,
                leave_date=datetime.date(2026, 7, 24),
                reason="Vacation / Scheduled Leave"
            )
            db.add(m_leave)
            logger.info("   ✓ Seeded StaffLeave for Marcus Johnson on 2026-07-24")
        ...
        docs = [
            ...
            KnowledgeDocument(
                ...
                content=(
                    "SalonAI stylists and staff are required to wear fashionable, professional all-black attire. "
                    "Clothing must be clean, free of rips or stains, and suitable for a luxury salon environment. "
                    "Closed-toe, comfortable shoes are mandatory for safety and ergonomics. "
                    "Stylists should express their personal style through their hair, makeup, and accessories, "
                    "while maintaining the professional all-black dress code."
                ),
                version=1,
                is_active=True,
                is_deleted=False
            ...
            KnowledgeDocument(
                id=uuid.uuid4(),
                title="Stylist Opening & Morning SOP Duties",
                document_type="policies",
                content=(
                    "Stylists scheduled for the morning shift must arrive 15 minutes prior to opening. "
                    "Opening duties include: turning on all lights and music, sanitizing and prepping styling stations, "
                    "checking the daily booking calendar for any morning appointments, stocking clean towels and capes, "
                    "and ensuring retail product displays are neat and fully stocked."
                ),
                version=1,
                is_active=True,
                is_deleted=False
            ...
            KnowledgeDocument(
                id=uuid.uuid4(),
                title="Staff Commission and Tips Policy",
                document_type="policies",
                content=(
                    "Stylists earn a base salary plus a 40% commission on all completed services. "
                    "Additionally, stylists earn a 10% commission on any retail products sold. "
                    "All tips received from customers (either cash or card) are paid out 100% directly to "
                    "the stylist on their weekly paycheck, without any salon deductions."
                ),
                version=1,
                is_active=True,
                is_deleted=False
            ...
            KnowledgeDocument(
                id=uuid.uuid4(),
                title="Lateness and Schedule Swap Policy",
                document_type="policies",
                content=(
                    "Stylists must report any late arrivals at least 30 minutes before their scheduled shift. "
                    "If a stylist needs to swap a shift with another team member, a formal schedule swap request "
                    "must be submitted via the platform at least 48 hours in advance. Both stylists must agree "
                    "to the swap, and it must be approved by the salon manager to avoid scheduling conflicts."
                ),
                version=1,
                is_active=True,
                is_deleted=False
