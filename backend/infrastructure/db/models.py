...
class MemoryStatus(str, enum.Enum):
    """Status lifecycle of a curated memory"""
    ACTIVE = "active"
...
class ConsentClass(str, enum.Enum):
    """Consent or privacy classification of a memory"""
    STANDARD = "standard"
    SENSITIVE = "sensitive"
...
class CuratedMemory(BaseModel):
    """
    Represents a curated, durable domain memory.
    """
    __tablename__ = "curated_memories"

    tenant_id = Column(String(100), nullable=False, default="default", index=True)
    scope = Column(SQLEnum(MemoryScope, name="memory_scope"), nullable=False, index=True)
    owner_id = Column(String(255), nullable=True, index=True)  # owner/customer/staff/lead identifier
    agent_domains = Column(Text, nullable=True)  # comma-separated list of agent domains allowed to access
    content = Column(Text, nullable=False)
    source_event_id = Column(String(255), nullable=True)
    source_type = Column(String(100), nullable=True)
    status = Column(SQLEnum(MemoryStatus, name="memory_status"), default=MemoryStatus.ACTIVE, nullable=False, index=True)
    confidence = Column(Float, default=1.0, nullable=False)
    importance = Column(Float, default=0.5, nullable=False)
    consent_class = Column(SQLEnum(ConsentClass, name="consent_class"), default=ConsentClass.STANDARD, nullable=False)
    expires_at = Column(DateTime(timezone=True), nullable=True)
    faiss_key = Column(String(255), nullable=True)

    def __repr__(self) -> str:
        return f"<CuratedMemory scope={self.scope} status={self.status} tenant={self.tenant_id}>"



class ConversationSession(BaseModel):
    """Stores conversation state (turns, slots, pending bookings) in the database."""
    ...
    history = Column(Text, nullable=True)  # JSON-serialized list of turns
    unresolved_slots = Column(Text, nullable=True)  # JSON-serialized dict
    pending_booking = Column(Text, nullable=True)  # JSON-serialized dict
    metadata_json = Column(Text, nullable=True)  # JSON-serialized dict

    def __repr__(self) -> str:
        return f"<ConversationSession id={self.session_id} user={self.user_id}>"


class OutboxEvent(BaseModel):
    """Domain events stored for asynchronous processing (Outbox Pattern)."""
    ...
    def __repr__(self) -> str:
        return f"<OutboxEvent type={self.event_type} status={self.status}>"
