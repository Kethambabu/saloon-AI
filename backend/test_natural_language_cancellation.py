...
async def test_natural_language_cancellation():
    ...
    print("\n📝 STEP 1: Create an appointment to cancel")
    print("-" * 70)
    ...
    print("\n\n📝 STEP 2: Cancel appointment using NATURAL LANGUAGE description")
    print("-" * 70)
    ...
    try:
        ...
        if result.messages:
            ...
            if "cancelled" in full_response.lower() or "canceled" in full_response.lower():
                print("\n✅ Appointment cancellation succeeded with natural language!")
                return True
            elif "not found" in full_response.lower() or "invalid identifier" in full_response.lower():
                print("\n❌ Appointment not found or invalid identifier error")
                return False
            else:
                print("\n⚠️ Response unclear - check output above")
                return None
...
if __name__ == "__main__":
    ...
    if success is True:
        print("\n" + "="*70)
        print("FINAL RESULT: ✅ NATURAL LANGUAGE CANCELLATION TEST PASSED")
        print("="*70)
        sys.exit(0)
    elif success is False:
        print("\n" + "="*70)
        print("FINAL RESULT: ❌ NATURAL LANGUAGE CANCELLATION TEST FAILED")
        print("="*70)
        sys.exit(1)
    else:
        print("\n" + "="*70)
        print("FINAL RESULT: ⚠️ NATURAL LANGUAGE CANCELLATION TEST INCONCLUSIVE")
        print("="*70)
        sys.exit(2)
