...
def get_schedule(staff_id: str, date_str: Optional[str] = None, db: Optional[Any] = None) -> str:
    """
    Retrieve the schedule/appointments for a specific staff member on a target date.

    Args:
        staff_id: UUID string or name of the staff member.
        date_str: Target date in YYYY-MM-DD format. If not provided, defaults to today.
    """
    logger.info(f"[StaffTools] get_schedule for staff_id: {staff_id}, date_str: {date_str}")
    session = db or SessionLocal()
    try:
        from utils.entity_resolver import resolve_staff
        staff_uuid = resolve_staff(staff_id, session, raise_on_missing=True)
...
def get_today_schedule(staff_id: str, db: Optional[Any] = None) -> str:
    """
...
def get_next_customer(staff_id: str) -> str:
    ...
    try:
        from utils.entity_resolver import resolve_staff
        staff_uuid = resolve_staff(staff_id, db, raise_on_missing=True)
        from core.query_context import check_staff_access
        access_error = check_staff_access(staff_uuid)
        if access_error:
            return access_error
...
def recommend_services(customer_id: str) -> str:
    ...
    Args:
        customer_id: UUID string or name of the customer.
    """
    logger.info(f"[StaffTools] recommend_services for customer: {customer_id}")
    db = SessionLocal()
    try:
        from utils.entity_resolver import resolve_customer
        customer_uuid = resolve_customer(customer_id, db, raise_on_missing=True)
        recs = RecommendationService.get_customer_recommendations(db=db, customer_id=str(customer_uuid))
        if not recs:
