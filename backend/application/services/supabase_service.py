...
def get_supabase_client():
    ...
    try:
        ...
        # Validate that we actually have the service_role key and not the anon key
        try:
            # JWT payload is in the second segment (base64-encoded)
            parts = key.split(".")
            if len(parts) == 3:
                ...
                if jwt_role == "anon":
                    logger.critical(
                        "[SupabaseService] ❌ SUPABASE_SERVICE_ROLE_KEY is actually the ANON key (role=anon)! "
                        "Row-Level Security is NOT bypassed. "
                        "Get your REAL service_role key from Supabase Dashboard → Settings → API → Service Role Key "
                        "and update SUPABASE_SERVICE_ROLE_KEY in your .env file."
                    )
                elif jwt_role == "service_role":
                    logger.info("[SupabaseService] ✅ Service role key validated (role=service_role).")
        except Exception:
            pass  # JWT parsing is optional — don't block startup
