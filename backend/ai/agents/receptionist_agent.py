...
from ai.tools.discovery import (
...
from infrastructure.cache.response_cache import (
    response_cache, history_cache, catalog_cache,
    make_availability_key, make_history_key, make_services_key, make_staff_key,
    invalidate_customer_cache, invalidate_availability_cache
)
...
from infrastructure.db import Branch, Customer, Service, Staff
...
def get_query_customer_id() -> Optional[str]:
    ...
    if "[SYSTEM CUSTOMER CONTEXT:" in context:
        try:
            ...
            if len(parts) > 1:
                cust_id = parts[1].split("]")[0].split(",")[0].strip()
                return cust_id
        except Exception:
            pass
    return None
...
def extract_booking_entities_deterministic(text: str, full_query: str = "") -> Dict[str, Optional[str]]:
    """
    Extracts booking entities (service, branch, staff, date, time) deterministically from text
    supporting tab-separated values, quotes, mixed casing, and messy user inputs.
    """
    entities = {"service": None, "branch": None, "stylist": None, "date": None, "time": None}
    if not text:
        return entities
    ...
    # 1. Date extraction: 2026-07-24, 24-07-2026, 24/07/2026, tomorrow, today, etc.
    m_iso = re.search(r"\b(\d{4}-\d{2}-\d{2})\b", cleaned)
    if m_iso:
        entities["date"] = m_iso.group(1)
    else:
        m_dmy = re.search(r"\b(\d{1,2})[-/](\d{1,2})[-/](\d{4})\b", cleaned)
        if m_dmy:
            d, m, y = int(m_dmy.group(1)), int(m_dmy.group(2)), int(m_dmy.group(3))
            if d > 12 and 1 <= m <= 12 and 1 <= d <= 31:
                entities["date"] = f"{y:04d}-{m:02d}-{d:02d}"
            elif 1 <= m <= 12 and 1 <= d <= 31:
                entities["date"] = f"{y:04d}-{m:02d}-{d:02d}"
        else:
            for rel in ["today", "tomorrow", "yesterday", "day after tomorrow", "next monday", "next tuesday", "next wednesday", "next thursday", "next friday", "next saturday", "next sunday"]:
                if rel in cleaned_lower:
                    entities["date"] = rel
                    break

    # 2. Time extraction: e.g. 12pm, 12:00, 9am, 5:30 pm, slot 12pm
    m_time = re.search(r"\b(\d{1,2}(?::\d{2})?\s*(?:am|pm))\b", cleaned_lower)
    if m_time:
        entities["time"] = m_time.group(1)
    else:
        m_time_24 = re.search(r"\b(\d{2}:\d{2})\b", cleaned)
        if m_time_24:
            entities["time"] = m_time_24.group(1)

    # 3. Known entity matching against database / catalog
    db = SessionLocal()
    try:
        services = db.query(Service).filter(Service.is_active == True).all()
        for s in services:
            if s.name.lower() in cleaned_lower:
                entities["service"] = s.name
                break
        
        branches = db.query(Branch).filter(Branch.is_active == True).all()
        for b in branches:
            if b.name.lower() in cleaned_lower:
                entities["branch"] = b.name
                break

        staff_members = db.query(Staff).filter(Staff.is_active == True).all()
        for st in staff_members:
            if st.full_name.lower() in cleaned_lower or (st.first_name and st.first_name.lower() in cleaned_lower and len(st.first_name) > 2):
                entities["stylist"] = st.full_name
                break
    except Exception as ex:
        logger.warning(f"Deterministic entity matching error: {ex}")
    finally:
        db.close()
...
def repair_date(date_input: Any) -> str:
    """Automatically resolve relative date keywords and absolute date strings (e.g., 'june 8th 2026', '24-07-2026', '2026-07-24') to YYYY-MM-DD."""
    if not date_input:
        return get_query_base_date()
    
    date_str = str(date_input).strip(' \t\n\r"\'`')
    if not date_str or _is_valid_uuid(date_str):
        return get_query_base_date()
    ...
    m_iso = re.search(r"\b(\d{4}-\d{2}-\d{2})\b", date_str)
    if m_iso:
        return m_iso.group(1)

    m_dmy = re.search(r"\b(\d{1,2})[-/](\d{1,2})[-/](\d{4})\b", date_str)
    if m_dmy:
        d, m, y = int(m_dmy.group(1)), int(m_dmy.group(2)), int(m_dmy.group(3))
        if d > 12 and 1 <= m <= 12 and 1 <= d <= 31:
...
def repair_branch(branch_id: Any) -> str:
    ...
    branch_id_clean = str(branch_id).strip(' \t\n\r"\'`')
    db = SessionLocal()
    try:
        resolved = resolve_branch(branch_id_clean, db, raise_on_missing=False)
        if resolved:
            return str(resolved)
        first_b = db.query(Branch).filter(Branch.is_active == True).first()
        return str(first_b.id) if first_b else str(branch_id_clean)
    finally:
        db.close()
...
def repair_service(service_id: Any) -> str:
    ...
    service_id_clean = str(service_id).strip(' \t\n\r"\'`')
    db = SessionLocal()
    try:
        resolved = resolve_service(service_id_clean, db, raise_on_missing=False)
        if resolved:
            return str(resolved)
        first_s = db.query(Service).filter(Service.is_active == True).first()
        return str(first_s.id) if first_s else str(service_id_clean)
    finally:
        db.close()
...
def repair_staff(staff_id: Any, branch_id: str = None) -> Optional[str]:
    ...
    if not staff_id or _is_placeholder_value(staff_id) or str(staff_id).lower().strip(' \t\n\r"\'`') in ["any", "none", "auto", "default"]:
        return None
        
    staff_id_clean = str(staff_id).strip(' \t\n\r"\'`')
    db = SessionLocal()
    try:
        resolved = resolve_staff(staff_id_clean, db, raise_on_missing=False)
        if resolved:
            return str(resolved)
        return None
    finally:
...
def _get_customer_memory(customer_id: str) -> Tuple[str, Optional[str], Optional[str]]:
    ...
    try:
        ...
        from infrastructure.db.models import Appointment, AppointmentStatus
...
def find_matching_active_appointment(customer_id: str, intent_json: Dict[str, Any], query_text: str) -> Optional[str]:
    ...
    db = SessionLocal()
    try:
        from infrastructure.db import Appointment, AppointmentStatus, Service, Branch, Staff
...
def get_available_staff(
    ...
    def run():
        ...
        val = validate_appointment_datetime(date, time)
        if not val["valid"]:
            return val["reason"]

        from datetime import datetime, time as dt_time, timedelta, timezone
        from infrastructure.db.models import Staff, Appointment, AppointmentStatus
        from infrastructure.db.database import SessionLocal
        from application.services.appointment_service import is_staff_on_leave
...
def check_stylist_availability(
    ...
    Uses an in-memory TTL cache (120 seconds) to avoid repeat DB queries.
    """
    import concurrent.futures
    def run():
        from application.services.datetime_validation import validate_appointment_datetime
        val = validate_appointment_datetime(date)
        if not val["valid"]:
            return val["reason"]
        ...
        # ── Cache lookup ─────────────────────────────────────────────────
        cache_key = make_availability_key(repaired_branch, repaired_date, repaired_staff, repaired_service)
        cached = response_cache.get(cache_key)
        if cached is not None:
            logger.debug("[Cache] availability HIT for %s", cache_key)
            return cached
        ...
        response_cache.set(cache_key, result, ttl=90)  # 90-second TTL
        logger.debug("[Cache] availability SET for %s", cache_key)
        return result
    with concurrent.futures.ThreadPoolExecutor(max_workers=1) as executor:
        future = executor.submit(run)
        try:
...
def book_new_appointment(
    ...
    def run():
        ...
        from ai.workflows.booking_workflow import book_appointment_workflow
    ...
    def run():
        ...
        if not appt_str or _is_placeholder_value(appt_str) or not _is_valid_uuid(appt_str):
            ...
            if sys_c:
                db = SessionLocal()
                try:
                    from infrastructure.db import Appointment, AppointmentStatus
        ...
        sys_c = get_query_customer_id()
        from ai.workflows.booking_workflow import cancel_appointment_workflow
...
def reschedule_existing_appointment(appointment_id: str, new_start_time: str) -> str:
    ...
    def run():
        ...
        sys_c = get_query_customer_id()
        from ai.workflows.booking_workflow import reschedule_appointment_workflow
...
def check_customer_booking_history(customer_id: str) -> str:
    ...
    Uses an in-memory TTL cache (300 seconds) to avoid repeat DB queries.
    Invalidated automatically on any booking mutation.
    """
    import concurrent.futures
    def run():
        repaired_cust = repair_customer(customer_id)
        ...
        # ── Cache lookup ─────────────────────────────────────────────────
        cache_key = make_history_key(repaired_cust)
        cached = history_cache.get(cache_key)
        if cached is not None:
            logger.debug("[Cache] history HIT for customer %s", repaired_cust)
            return cached
        ...
        if res.get("success"):
            ...
            result = str(TypedAgentResponse.appointment_history(records).to_agent_dict())
        else:
            result = str(TypedAgentResponse.general_error(res.get("error", "Failed to retrieve booking history")).to_agent_dict())
        history_cache.set(cache_key, result, ttl=300)
        logger.debug("[Cache] history SET for customer %s", repaired_cust)
        return result
    with concurrent.futures.ThreadPoolExecutor(max_workers=1) as executor:
        future = executor.submit(run)
        try:
...
- NEVER alter or advance dates provided by the user. If the user specified '20-07-2026', extract date as '20-07-2026'. Do NOT change past dates to today or future dates.
- Output ONLY valid JSON. No explanation.
- Phrasing starting with or containing "No...", "Actually...", "I meant...", "Instead...", "Change to...", "Make it..." indicate corrections or updates to booking details. The action MUST be "book" (or "reschedule" if modifying a specific active appointment ID), NEVER "cancel".
- Action "cancel" MUST ONLY be used when the user explicitly requests to cancel an appointment (e.g. "cancel my appointment", "cancel booking").
- Distinguish 'history' vs 'availability':
  ...
  * Use 'availability' when the user asks if the salon or a stylist has open/free slots, is available, or has openings for booking a new appointment (e.g., "is Marcus Johnson available", "is there any slot open", "are you free", "check openings")."""

_PLANNER_CONFIDENCE_THRESHOLD = 0.70  # Below this -> ask a clarifying question


def is_cancellation_request(text: str) -> bool:
    """Check if user message explicitly requests appointment cancellation."""
    if not text:
        return False
    t = text.lower().strip()
    cancel_keywords = [
        "cancel my", "cancel appointment", "cancel booking", "cancel slot",
        "drop my booking", "want to cancel", "please cancel", "cancel current",
        "abort booking", "abort appointment"
    ]
    if any(k in t for k in cancel_keywords):
        return True
    if t in ("cancel", "cancel it", "cancel booking", "cancel appointment"):
        return True
    return False


def is_correction_request(text: str) -> bool:
    """Check if user message is a correction or detail update to a booking."""
    if not text:
        return False
    t = text.lower().strip()
    correction_prefixes = [
        "no,", "no ", "actually", "i meant", "instead", "change to", "make it",
        "rather", "update to", "no i want", "no, i want", "change date", "change time", "change stylist"
    ]
    return any(t.startswith(p) or f" {p}" in t for p in correction_prefixes)


def print_supabase_status() -> Dict[str, Any]:
    """
    Mandatory check that prints the current Supabase status (Database & Storage) at start.
    """
    try:
        from core.config import get_settings
        from infrastructure.db.database import is_using_fallback, check_db_health
        settings = get_settings()

        db_url_set = bool(settings.database_url and settings.database_url.strip())
        using_sqlite = is_using_fallback()
        db_healthy = check_db_health() if (db_url_set and not using_sqlite) else False
        ...
        status_msg = (
            "\n" + "=" * 60 + "\n"
            "📊 [SUPABASE STATUS AT RECEPTIONIST AGENT START]\n"
            f"  • Database URL Configured : {'Yes' if db_url_set else 'No'}\n"
            f"  • PostgreSQL Connection   : {'✅ CONNECTED & HEALTHY' if db_healthy else ('⚠️ OFF-LINE (Using SQLite Fallback)' if using_sqlite else '❌ DISCONNECTED')}\n"
            f"  • Supabase Storage        : {'✅ CONFIGURED' if storage_configured else '⚠️ NOT CONFIGURED'}\n"
            + "=" * 60 + "\n"
        )
        try:
            ...
            pass
        logger.info(
            f"[Supabase Status] DB Configured={db_url_set}, Healthy={db_healthy}, SQLite Fallback={using_sqlite}, Storage Configured={storage_configured}"
        )
        return {
        ...
        err_msg = f"[Supabase Status Check Error]: {e}"
        try:
...
def sort_queue_cheap_first(queue: List[Dict]) -> List[Dict]:
    """
    Sort LLM fallback queue preserving strict provider tier priority for Receptionist Agent:
    1. Primary: Hugging Face models
    2. Fallback 1: Groq models
    3. Fallback 2: Gemini models
    """
    def get_rank(item: Dict) -> int:
        provider = (item.get("provider") or "").lower()
        model = (item.get("model") or "").lower()

        # Primary Tier (1-9): Hugging Face
        if provider == "huggingface" or "huggingface" in model or "qwen" in model:
            return 1

        # Fallback Tier 1 (10-19): Groq
        if provider == "groq" or "llama" in model:
            if "8b" in model:
                return 10
            return 11

        # Fallback Tier 2 (20-29): Gemini (Next fallback)
    ...
    return sorted(queue, key=get_rank)
...
async def _run_planner(
    ...
    # Sort the fallback queue cheap first specifically for planner
    sorted_queue = sort_queue_cheap_first(fallback_queue)

    for tier in sorted_queue:
...
class ReceptionistAgent(Agent):
    ...
    def __init__(self, name: str = "Clara", role: str = "AI Salon Receptionist"):
        ...
        # Mandatory Supabase status print on Receptionist Agent start
    ...
    async def process(self, input_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Priority 4: Direct discovery catalog browser rules (Bypass planner to save tokens)
        ...
        if is_discovery_query:
            logger.info("⚡ Discovery query detected. Bypassing planner.")
            plan = {
                "confidence": 1.0,
                "tasks": [{
                    "action": "general_chat",
        ...
        session = state_svc.get_or_create(session_id, user_id=user_id, user_role=user_role)

        # Check if latest_message is an explicit confirmation response or pure time / active flow continuation
        aff_words = ["yes", "confirm", "sure", "proceed", "ok", "okay", "yep", "yeah", "cancel it", "do it"]
        is_affirmative = any(w in latest_message.lower().strip(' \t\n\r"\'`') for w in aff_words)
        _is_time_reply = bool(re.search(r"^\s*\d{1,2}(?::\d{2})?\s*(?:am|pm)?\s*$", latest_message, re.IGNORECASE))

        if session.pending_booking or _is_time_reply or is_affirmative:
            plan_confidence = 1.0

        # ── STEP 2: Confidence gating — ask clarifying question if ambiguous ──────
        if plan_confidence < _PLANNER_CONFIDENCE_THRESHOLD and not is_affirmative:
            clarify_msg = (
        ...
        # Deterministic Entity Extractor (handles tab-separated strings, quotes, mixed casing)
        det_entities = extract_booking_entities_deterministic(latest_message, query)
        for k, v in det_entities.items():
            if v and not _is_placeholder_value(v):
                intent_json[k] = v

        # Clean all intent_json values from surrounding quotes/tabs/whitespace
        for k in ["service", "branch", "stylist", "date", "time"]:
            if intent_json.get(k):
                clean_val = str(intent_json[k]).strip(' \t\n\r"\'`')
                intent_json[k] = clean_val if (clean_val and not _is_placeholder_value(clean_val)) else None

        # Safety fallback: extract service keyword if omitted
        if not intent_json.get("service") or _is_placeholder_value(intent_json.get("service", "")):
            for s_keyword in ["hair spa", "haircut", "facial", "massage", "beard styling", "manicure", "pedicure", "coloring", "hair color", "hair styling"]:
                if s_keyword in latest_message.lower() or s_keyword in query.lower():
                    intent_json["service"] = s_keyword.title()
                    break

        # ── DETERMINISTIC STATE MACHINE & CORRECTION OVERRIDES ───────────────────
        is_corr = is_correction_request(latest_message)
        is_canc = is_cancellation_request(latest_message)

        # Force intent to "book" if user message contains booking keywords or if user is correcting/continuing an active booking
        booking_keywords = ["book", "appoitment", "appointment", "schedule", "reserve", "slot", "make it", "change to", "no i want"]
        msg_clean_lower = latest_message.lower().strip(' \t\n\r"\'`')
        is_resched_msg = intent_json.get("intent") == "reschedule" or "reschedule" in msg_clean_lower
        if any(re.search(r"\b" + re.escape(w) + r"\b", msg_clean_lower) for w in booking_keywords) and not is_canc and not is_resched_msg:
            intent_json["intent"] = "book"

        if session.pending_booking.get("_awaiting_reschedule"):
            intent_json["intent"] = "reschedule"
        elif session.pending_booking.get("_awaiting_cancellation"):
            intent_json["intent"] = "cancel"
        elif (is_corr or session.pending_booking) and not is_canc and intent_json.get("intent") not in ("cancel", "reschedule"):
            intent_json["intent"] = "book"
            if session.booking_state in (BookingState.CONFIRMED, BookingState.CLOSED):
                session.set_booking_state(BookingState.COLLECTING)
            if getattr(session, "last_booking", None):
                for k in ["service", "branch", "stylist"]:
                    if not intent_json.get(k) and not session.pending_booking.get(k):
                        if session.last_booking.get(k):
                            session.pending_booking[k] = session.last_booking[k]
                            intent_json[k] = session.last_booking[k]

        # Latest explicit booking parameters override previous state
        for key in ["service", "branch", "stylist", "date", "time"]:
            val = intent_json.get(key)
        ...
        for key in ["service", "branch", "stylist", "date", "time"]:
            if not intent_json.get(key) or _is_placeholder_value(intent_json.get(key, "")):
                if session.pending_booking.get(key):
                    intent_json[key] = session.pending_booking[key]
            ...
            for _hist_msg in reversed(history_user_lines):
                ...
                if not intent_json.get("date"):
                    ...
                    if _date_m:
                        ...
                        session.pending_booking["date"] = _date_m.group(1)
                if not intent_json.get("time"):
                    ...
                    if _time_m:
                        ...
                        session.pending_booking["time"] = _time_m.group(1)
        ...
        # ROUTING & BOOKING ENGINE (Deterministic State Machine)
        if intent == "book" and not is_discovery_query:
            ...
            is_repeat = any(w in query.lower() for w in ["same", "again", "repeat", "last appointment"]) or (service_input and "same" in str(service_input).lower())
            if is_repeat and cust_id:
                db = SessionLocal()
                try:
                    import uuid as _uuid_repeat
                    c_uuid = _uuid_repeat.UUID(str(cust_id)) if isinstance(cust_id, str) else cust_id
                    from infrastructure.db.models import Appointment, AppointmentStatus
                    last_appt = db.query(Appointment).filter(
                        Appointment.customer_id == c_uuid,
                        Appointment.status.in_([AppointmentStatus.COMPLETED, AppointmentStatus.CONFIRMED])
                    ).order_by(Appointment.start_time.desc()).first()
                    if last_appt:
                        if not service_input or "same" in str(service_input).lower() or _is_placeholder_value(service_input):
                            service_input = str(last_appt.service_id)
                        if (not stylist_input or _is_placeholder_value(stylist_input)) and last_appt.staff_id:
            ...
            # Auto-default branch if omitted and EXACTLY ONE active branch exists
            if not branch_input or _is_placeholder_value(branch_input):
                db = SessionLocal()
                try:
                    active_branches = db.query(Branch).filter(Branch.is_active == True).all()
                    if len(active_branches) == 1:
                        branch_input = str(active_branches[0].id)
                        session.pending_booking["branch"] = branch_input
                        intent_json["branch"] = branch_input
                except Exception as ex:
                    logger.warning(f"Error checking active branch count: {ex}")
                finally:
                    db.close()
            ...
            if not val_res["valid"]:
                ...
                suggestion_msg = ""
                # If the requested time today has already passed, proactively suggest
                # the next available slots today instead of just rejecting the request.
                if "already passed today" in val_res["reason"]:
                    try:
                        sugg_branch = repair_branch(branch_input)
                        sugg_service = repair_service(service_input)
                        sugg_staff = repair_staff(stylist_input or pref_stylist, sugg_branch)
                        sugg_date = adjust_past_date_today(repair_date(date_input), repair_time(time_input))
                        slots_data = check_stylist_availability(
                            branch_id=sugg_branch,
                            date=sugg_date,
                            staff_id=sugg_staff,
                            service_id=sugg_service
                        )
                        import ast as _ast
                        slots_dict = {}
                        try:
                            slots_dict = _ast.literal_eval(slots_data)
                        except Exception:
                            try:
                                slots_dict = json.loads(slots_data)
                            except Exception:
                                pass
                        raw_slots = slots_dict.get("slots") or (slots_dict.get("data") or {}).get("slots") or []
                        upcoming_times = []
                        for s in raw_slots:
                            start_iso = s.get("start_time", "")
                            try:
                                dt = datetime.fromisoformat(start_iso.replace("Z", "+00:00"))
                                upcoming_times.append(dt.strftime("%I:%M %p"))
                            except Exception:
                                pass
                        if upcoming_times:
                            suggestion_msg = f"\n\nHere are the next available slots today: {', '.join(upcoming_times[:5])}."
                        else:
                            suggestion_msg = "\n\nThere are no remaining slots available today. Please try booking for tomorrow or another future date."
                    except Exception as sug_exc:
                        logger.warning(f"Error building next-available-slot suggestion: {sug_exc}")
            ...
            # ── CONFIRMATION GATE (Run BEFORE availability check) ─────────────────
            # Check if this is an explicit confirmation of a previously shown summary
            is_explicit_confirmation = bool(
                re.search(r"\b(yes|confirm|proceed|sounds good|book it|please confirm|sure|go ahead|do it)\b", latest_message, re.IGNORECASE)
            )
            ...
            if not _already_awaiting and not is_explicit_confirmation:
                ...
                import ast as _ast_conf
                import json as _json_conf
                _slots_dict_conf = {}
                try:
                    _slots_dict_conf = _ast_conf.literal_eval(_temp_slots)
                except Exception:
                    try:
                        _slots_dict_conf = _json_conf.loads(_temp_slots)
                    except Exception:
                        pass
                ...
                try:
                    ...
                    try:
                        if repaired_service:
                            _svc_uid = _uuid_mod.UUID(str(repaired_service))
                    except ValueError:
                        pass
                    ...
                    try:
                        if repaired_branch:
                            _br_uid = _uuid_mod.UUID(str(repaired_branch))
                    except ValueError:
                        pass
                    ...
                    try:
                        if repaired_staff:
                            _st_uid = _uuid_mod.UUID(str(repaired_staff))
                    except ValueError:
                        pass

                    if _svc_uid:
                        _s_obj = _db_conf.query(Service).filter(Service.id == _svc_uid).first()
                        if _s_obj:
                            _svc_name = _s_obj.name
                            _duration_val = getattr(_s_obj, "duration_minutes", 30)
                            _price_val = getattr(_s_obj, "price", 0.0)
                    if _br_uid:
                        _b_obj = _db_conf.query(Branch).filter(Branch.id == _br_uid).first()
                        if _b_obj: _br_name = _b_obj.name
                    if _st_uid:
                        _st_obj = _db_conf.query(Staff).filter(Staff.id == _st_uid).first()
                ...
                session.pending_booking["_confirmed_branch"] = str(repaired_branch)
                session.pending_booking["_confirmed_staff"] = str(repaired_staff) if repaired_staff else None
                session.pending_booking["_confirmed_time_str"] = repaired_time_str
                session.set_booking_state(BookingState.AWAITING_CONFIRMATION)
                state_svc._save_session(session)

                _summary = (
                    f"I have checked availability and your slot is open! Here is your Booking Summary:\n\n"
                    f"• **Service**: {_svc_name}\n"
                    f"• **Stylist**: {_st_name}\n"
                    f"• **Branch**: {_br_name}\n"
                    f"• **Date**: {repaired_date}\n"
                    f"• **Time**: {repaired_time}\n"
                    f"• **Duration**: {_duration_val} mins\n"
                    f"• **Price**: ${_price_val}\n\n"
                ...
                return {
                    ...
                    "agent_name": self.name,
                    "response": f"{pref_str}\n\n{_summary}".strip(),
                    "provider": "booking_engine_confirmation_required"
                }

            # User confirmed — clear the gate flag and proceed to book
            session.pending_booking.pop("_awaiting_confirmation", None)
            # Restore confirmed entity IDs from session if available
            if session.pending_booking.get("_confirmed_service"):
                repaired_service = session.pending_booking.pop("_confirmed_service")
            if session.pending_booking.get("_confirmed_branch"):
                repaired_branch = session.pending_booking.pop("_confirmed_branch")
            if session.pending_booking.get("_confirmed_staff"):
                repaired_staff = session.pending_booking.pop("_confirmed_staff")
            if session.pending_booking.get("_confirmed_time_str"):
                repaired_time_str = session.pending_booking.pop("_confirmed_time_str")

            # ── STATE MACHINE: AVAILABILITY (now runs only when confirmed) ─────────
            session.set_booking_state(BookingState.AVAILABILITY)
            slots_data = check_stylist_availability(
                branch_id=repaired_branch,
                date=repaired_date,
            ...
            if not is_requested_slot_available:
                ...
                if repaired_staff:
                    db = SessionLocal()
                    try:
                        _st_uuid = uuid.UUID(str(repaired_staff))
                        st = db.query(Staff).filter(Staff.id == _st_uuid).first()
                        if st:
            ...
            # ── (Confirmation check already handled above; proceed to book) ──────
            ...
            try:
                invalidate_availability_cache(repaired_branch, repaired_date)
                invalidate_customer_cache(repaired_cust)
            except Exception as _ce:
                logger.debug("[Cache] invalidation error: %s", _ce)

            # Format confirmed response using strict summary structure
            db = SessionLocal()
            try:
                import uuid
                service_uuid = uuid.UUID(str(repaired_service)) if isinstance(repaired_service, (str, uuid.UUID)) else repaired_service
                branch_uuid = uuid.UUID(str(repaired_branch)) if isinstance(repaired_branch, (str, uuid.UUID)) else repaired_branch
                
                service_name = db.query(Service).filter(Service.id == service_uuid).first().name
                branch_name = db.query(Branch).filter(Branch.id == branch_uuid).first().name
                stylist_name = "Professional Stylist"
                if repaired_staff:
                    staff_uuid = uuid.UUID(str(repaired_staff)) if isinstance(repaired_staff, (str, uuid.UUID)) else repaired_staff
                    st = db.query(Staff).filter(Staff.id == staff_uuid).first()
                    if st:
                        stylist_name = f"{st.first_name} {st.last_name}"
                price_val = db.query(Service).filter(Service.id == service_uuid).first().price
            except Exception as e:
                logger.warning(f"Error building confirmation response details: {e}")
                service_name = "Styling Session"
            ...
            # Memory retention: store last appointment & preferences
            session.update_customer_preferences(
                preferred_stylist=stylist_name,
                preferred_service=service_name,
                preferred_branch=branch_name
            )
            session.update_last_appointment({
                "service": service_name,
                "stylist": stylist_name,
                "branch": branch_name,
                "date": repaired_date,
            ...
            # ── STATE MACHINE: CONFIRMED -> CLOSED ─────────────────────────────────
            # Requirement 5: Clear workflow state after completion to avoid stale context
            session.set_booking_state(BookingState.CONFIRMED)
            session.clear_pending_booking()  # transitions to BookingState.CLOSED and resets pending_booking
            state_svc._save_session(session)
            ...
            _already_cancelling = session.pending_booking.get("_awaiting_cancellation", False)
            is_explicit_cancel_confirm = bool(
                re.search(r"\b(yes|confirm|proceed|cancel it|sure|do it|go ahead|yes cancel)\b", latest_message, re.IGNORECASE)
            )

            if not _already_cancelling and not is_explicit_cancel_confirm:
                # Step 1: Check appointment & ask confirmation
                appt_details_str = ""
                if appt_id:
                    db_c = SessionLocal()
                    try:
                        import uuid as _uc
                        _a_uuid = _uc.UUID(str(appt_id))
                        _a_obj = db_c.query(Appointment).filter(Appointment.id == _a_uuid).first()
                        if _a_obj:
                            if _a_obj.status == AppointmentStatus.CANCELLED:
                                return {
                                    "success": True,
                                    "agent_name": self.name,
                                    "response": f"{pref_str}\n\nThis appointment is already cancelled.".strip(),
                                    "provider": "booking_engine"
                                }
                            _s_name = _a_obj.service.name if _a_obj.service else "Service"
                            _b_name = _a_obj.branch.name if _a_obj.branch else "Branch"
                            _d_str = _a_obj.start_time.strftime("%Y-%m-%d")
                            _t_str = _a_obj.start_time.strftime("%I:%M %p")
                            appt_details_str = f" for {_s_name} at {_b_name} on {_d_str} at {_t_str}"
                    except Exception:
                        pass
                    finally:
                        db_c.close()

                session.pending_booking["_awaiting_cancellation"] = True
                session.pending_booking["_cancel_appt_id"] = str(appt_id) if appt_id else None
                state_svc._save_session(session)

                return {
                    "success": True,
                    "agent_name": self.name,
                    "response": f"{pref_str}\n\nAre you sure you want to cancel this appointment{appt_details_str}?".strip(),
                    "provider": "booking_engine_cancellation_confirmation"
                }

            # Step 2: User confirmed cancellation — execute & verify
            session.pending_booking.pop("_awaiting_cancellation", None)
            target_cancel_id = session.pending_booking.pop("_cancel_appt_id", None) or appt_id

            cancel_res = cancel_existing_appointment(appointment_id=target_cancel_id)
            
            # Verify DB update
            db_c_verify = SessionLocal()
            try:
                if target_cancel_id:
                    import uuid as _ucv
                    _a_uuid = _ucv.UUID(str(target_cancel_id))
                    _a_obj = db_c_verify.query(Appointment).filter(Appointment.id == _a_uuid).first()
                    if _a_obj and _a_obj.status == AppointmentStatus.CANCELLED:
                        logger.info("Database transaction verified: Appointment %s is marked CANCELLED", target_cancel_id)
            except Exception as c_ve:
                logger.warning(f"Error during cancel DB verification: {c_ve}")
            finally:
                db_c_verify.close()

            formatted_cancel = format_receptionist_tool_output("cancel", cancel_res)

            # Invalidate history cache after cancellation
            try:
                invalidate_customer_cache(repaired_cust)
            except Exception as _ce:
                logger.debug("[Cache] invalidation error after cancel: %s", _ce)

            session.set_booking_state(BookingState.CLOSED)
            state_svc._save_session(session)
            ...
            _already_rescheduling = session.pending_booking.get("_awaiting_reschedule", False)
            is_explicit_resched_confirm = bool(
                re.search(r"\b(yes|confirm|proceed|change it|sure|do it|go ahead)\b", latest_message, re.IGNORECASE)
            )

            if not _already_rescheduling and not is_explicit_resched_confirm:
                # Step 1: Display Old Appointment vs New Appointment comparison & ask confirmation
                _old_svc, _old_br, _old_st, _old_time = "Styling Session", "Salon Branch", "Stylist", "Current Slot"
                if appt_id:
                    db_r = SessionLocal()
                    try:
                        import uuid as _ur
                        _a_uuid = _ur.UUID(str(appt_id))
                        _a_obj = db_r.query(Appointment).filter(Appointment.id == _a_uuid).first()
                        if _a_obj:
                            _old_svc = _a_obj.service.name if _a_obj.service else _old_svc
                            _old_br = _a_obj.branch.name if _a_obj.branch else _old_br
                            _old_st = _a_obj.staff.full_name if _a_obj.staff else _old_st
                            _old_time = _a_obj.start_time.strftime("%Y-%m-%d %I:%M %p")
                    except Exception:
                        pass
                    finally:
                        db_r.close()

                session.pending_booking["_awaiting_reschedule"] = True
                session.pending_booking["_resched_appt_id"] = str(appt_id) if appt_id else None
                session.pending_booking["_resched_start_time"] = new_start_time
                state_svc._save_session(session)

                resched_summary = (
                    f"Here is your Rescheduling Summary:\n\n"
                    f"**Old Appointment**:\n"
                    f"• Service: {_old_svc}\n"
                    f"• Branch: {_old_br}\n"
                    f"• Stylist: {_old_st}\n"
                    f"• Date & Time: {_old_time}\n\n"
                    f"↓\n\n"
                    f"**New Appointment**:\n"
                    f"• Service: {_old_svc}\n"
                    f"• Branch: {_old_br}\n"
                    f"• Stylist: {_old_st}\n"
                    f"• Date & Time: {new_date} {new_time}\n\n"
                    f"Would you like me to confirm this change?"
                )
                return {
                    "success": True,
                    "agent_name": self.name,
                    "response": f"{pref_str}\n\n{resched_summary}".strip(),
                    "provider": "booking_engine_reschedule_confirmation"
                }

            # Step 2: User confirmed rescheduling — execute & verify
            session.pending_booking.pop("_awaiting_reschedule", None)
            target_resched_id = session.pending_booking.pop("_resched_appt_id", None) or appt_id
            target_start_time = session.pending_booking.pop("_resched_start_time", None) or new_start_time

            resched_res = reschedule_existing_appointment(appointment_id=target_resched_id, new_start_time=target_start_time)
            
            # Verify DB update
            db_r_verify = SessionLocal()
            try:
                if target_resched_id:
                    import uuid as _urv
                    _a_uuid = _urv.UUID(str(target_resched_id))
                    _a_obj = db_r_verify.query(Appointment).filter(Appointment.id == _a_uuid).first()
                    if _a_obj:
                        logger.info("Database transaction verified: Appointment %s updated in DB to %s", target_resched_id, _a_obj.start_time)
            except Exception as r_ve:
                logger.warning(f"Error during reschedule DB verification: {r_ve}")
            finally:
                db_r_verify.close()

            formatted_resched = format_receptionist_tool_output("reschedule", resched_res)

            # Memory update after reschedule
            session.update_last_appointment({
                "date": new_date,
                "time": new_time,
                "status": "Confirmed"
        ...
        for idx, tier in enumerate(fallback_queue, 1):
            ...
            for attempt in range(1, max_attempts + 1):
                ...
                try:
                    ...
                    # ── Python-Based Formatter (replaces secondary LLM formatter call) ──
                    # Detects raw JSON tool output and formats it without any LLM API call.
                    # This eliminates 1-2 API calls per request, saving tokens and latency.
                    if (response_stripped.startswith("{") or response_stripped.startswith("[") or response_stripped.startswith("{'")) or (
                            "success" in response_stripped.lower() and ("true" in response_stripped.lower() or "false" in response_stripped.lower())):
                        data = None
                        try:
                            ...
                            try:
                                import ast as _ast2
                                data = _ast2.literal_eval(response_stripped)
                            except Exception:
                                data = None

                        if isinstance(data, dict):
                            response_type_field = str(data.get("response_type", "")).lower()
                            resource_field = str(data.get("resource", "")).lower()
                            operation_field = str(data.get("operation", "")).lower()
                            error_field = data.get("error")

                            # 1. Error responses
                            if error_field and str(error_field).strip():
                                err_clean = str(error_field).replace("database", "system").replace("UUID", "reference").replace("SQL", "system").replace("sql", "system")
                                response_text = (
                                    f"I apologize, but I encountered an issue: {err_clean} "
                                    "Would you like to try a different time, date, or stylist?"
                                )

                            # 2. Appointment history
                            elif (resource_field == "appointments" and operation_field == "select") or response_type_field == "appointment_history":
                                records = data.get("data") or data.get("records") or data.get("history") or []
                                if isinstance(records, dict):
                                    records = records.get("records") or records.get("history") or []
                                if not records:
                                    response_text = "You do not have any past styling appointments on record with us."
                                else:
                                    lines = ["Here is your styling history:\n"]
                                    for appt in records[:20]:
                                        if not isinstance(appt, dict):
                                            continue
                                        start_str = appt.get("start_time", "")
                                        try:
                                        ...
                                        staff = appt.get("staff_name") or "Professional Stylist"
                                        branch = appt.get("branch_name") or "SalonAI Lounge"
                                        status = str(appt.get("status", "CONFIRMED")).upper()
                                        lines.append(f"• **{svc}** with {staff} at {branch} on {nice_time} ({status})")
                                    response_text = "\n".join(lines)

                            # 3. Services list
                            elif resource_field == "services" and operation_field == "select":
                                items = data.get("data") or []
                                if not items:
                                    response_text = "Here are our available services. Please ask me for the latest catalog!"
                                else:
                                    ...
                                    for s in items:
                                        ...
                                        lines.append(f"• **{name}**{dur_str} — ${price}")
                                        if desc:
                                            lines.append(f"  _{desc}_")
                                    response_text = "\n".join(lines)

                            # 4. Reviews list
                            elif (resource_field == "reviews" and operation_field == "select") or response_type_field in ("review_submitted", "review_escalated"):
                                items = data.get("data") or []
                                if not items:
                                    response_text = "You do not have any reviews on record with us."
                                else:
                                    lines = ["Here are your reviews:\n"]
                                    for rv in items[:10]:
                                        if not isinstance(rv, dict):
                                            continue
                                        rating = rv.get("rating", "?")
                                        comment = rv.get("comment") or rv.get("review_text", "")
                                        svc = rv.get("service_name", "")
                                        stars = "⭐" * int(rating) if str(rating).isdigit() else str(rating)
                                        lines.append(f"• {stars} — {svc}: {comment[:120]}")
                                    response_text = "\n".join(lines)

                            # 5. Generic successful confirmation — format as booking summary
                            elif data.get("success") and data.get("appointment_id") and operation_field != "select":
                                svc_n = data.get("service_name", "Styling Session")
                                br_n = data.get("branch_name", "Main Salon")
                                st_n = data.get("stylist_name") or data.get("staff_name", "Stylist")
                                st_dt = data.get("start_time", "")
                                pr = data.get("price", "")
                                upsell_list = []
                                if "haircut" in svc_n.lower():
                                    upsell_list = ["Hair Spa ($55)", "Special Head Massage ($25)", "Professional Beard Styling ($35)"]
                                elif "massage" in svc_n.lower():
                                    upsell_list = ["Luxury Facial Treatment ($120)", "Himalayan Sea Salt foot wash ($30)"]
                                else:
                                    upsell_list = ["Signature Precision Haircut ($85)", "Special Head Massage ($25)"]
                                try:
                                    dt2 = datetime.fromisoformat(str(st_dt).replace("Z", "+00:00"))
                                    nice_dt = dt2.strftime("%A, %B %d, %Y at %I:%M %p")
                                except Exception:
                                    nice_dt = str(st_dt)
                                response_text = (
                                    f"Appointment Summary\n\n"
                            ...
                            # else: keep original response_text unchanged (LLM already gave readable text)
                        logger.info("[Formatter] Python-based format applied (no secondary LLM call). len=%d", len(response_text))
                    ...
                    from core.renderer import render_response as _render_response
