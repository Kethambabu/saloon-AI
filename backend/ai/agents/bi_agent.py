...
# ---------------------------------------------------------------------------
def get_dashboard_summary(period: str = "today") -> str:
    """
    Retrieve core business performance indicators: revenue, completed bookings, 
    new customers, lead conversion rate, average rating, and upsell revenue 
    for a specific period (e.g. 'today', 'yesterday', 'weekly', 'monthly', 'yearly').
    """
    logger.info(f"[BIAgent] Tool call: get_dashboard_summary(period={period})")
    db = SessionLocal()
    try:
        res = AnalyticsService.get_dashboard_summary(db, period=period)
        return str(res)
    finally:
        db.close()
...
def get_database_schema() -> str:
    """
    Retrieve the database schema details including table definitions, column types,
    and valid enum values for raw SQL query generation.
    """
    logger.info("[BIAgent] Tool call: get_database_schema()")
    schema_info = """
### Salon Database Table Schemas:
...
1. `mcp_read(resource="dashboard", filters={"period": "today"|"yesterday"|"weekly"|"monthly"|"yearly"})` - Core aggregates (revenue, conversions, active bookings, ratings) for the specified period.
2. `mcp_read(resource="revenue")` - Revenue distributions (service, branch, staff) and cards (including today_revenue and yesterday_revenue).
...
11. `mcp_read(resource="db_schema")` - Dynamically retrieves the database schema layout, tables, and relationships. Use this before writing custom raw SQL.
12. `mcp_read(resource="raw_sql", filters={"sql": "SELECT..."})` - Custom SELECT queries on allowed schemas.
13. `search_knowledge_base(domain="policies"|"faq"|"bi_memory", query=...)` - Search general rules or past business intelligence memories.
14. `execute_transaction(action="trigger_returning_cohort_reminders", parameters={})` - Manually trigger daily cohort loyalty reminders.
...
```
Ensure that:
1. The JSON structure is correct and valid.
2. The values are numbers (integers or floats), NOT strings.
3. The block is at the very end of your message.
        ...
        try:
            ...
            if vis_marker_start in response_text:
                try:
                    ...
                    logger.warning(f"[BIAgent] Failed to parse json_vis block: {ex}")

            # Store assistant response (stripped of visual block)
