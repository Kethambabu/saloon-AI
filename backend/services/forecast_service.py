...
class ForecastService:
    ...
    def get_forecast_metrics(db: Session) -> Dict[str, Any]:
        ...
        base_rev = float(avg_revenue) if avg_revenue is not None else 0.0
        base_appts = float(avg_appts) if avg_appts is not None else 0.0
        base_conv = float(avg_conv * 100.0) if avg_conv is not None else 0.0
        base_upsells = float(avg_upsells) if avg_upsells is not None else 0.0
        
        # Estimate growth modifier (+8% forecast)
        predicted_growth = 0.08
        
        return {
            "expected_revenue": round(base_rev * 30 * (1.0 + predicted_growth), 2) if avg_revenue is not None else 0.0,
            "expected_appointments": int(base_appts * 30 * (1.0 + predicted_growth)) if avg_appts is not None else 0,
            "expected_leads": 0,
            "expected_conversion": round(base_conv, 1),
            "expected_upsell_revenue": round(base_upsells * 30 * (1.0 + predicted_growth), 2) if avg_upsells is not None else 0.0,
